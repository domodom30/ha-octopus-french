...
class OctopusFrenchClient:
        ...
        try:
            async with timeout(30):
                ...
                if (
                    ...
                    and response["data"].get("obtainKrakenToken")
                ):
    ...
    async def _graphql_query(
        self, query: str, variables: dict = None, auth_required: bool = True
    ) -> dict | None:
        ...
        try:
            async with timeout(30):
                async with self._session.post(
                    GRAPH_QL_ENDPOINT, json=payload, headers=headers
                ) as response:
    ...
    async def get_data_ledgers(
        ...
        for ledger in ledgers:
            ...
            if ledger_type == "FRA_GAS_LEDGER" and meter_point_id:
                readings = await self.get_data_gas(
                    account_number, meter_point_id, limit=100
                )
            ...
            if ledger_type == "FRA_ELECTRICITY_LEDGER" and meter_point_id:
                readings = await self.get_data_electricity(
                    account_number, meter_point_id, limit=100
                )
                # Ajouter les lectures au ledger
                ledger_copy["additional_data"] = {"readings": readings}
        ...
        return enhanced_ledgers or []

    async def get_data_electricity(
        self, account_number: str, prm_id: str, limit: int = 10
    ) -> list[dict] | None:
        """Get electricity readings for a specific PRM ID."""
        response = await self._graphql_query(
            QUERY_ELECTRICITY_READINGS,
            {"accountNumber": account_number, "prmId": prm_id, "first": limit},
        )

        if (
            response
            and "data" in response
            and response["data"].get("electricityReading")
        ):
            readings = []
    ...
    async def get_data_gas(
        self, account_number: str, pce_ref: str, limit: int = 10
    ) -> list[dict] | None:
        """Get gas readings for a specific PCE reference."""
        response = await self._graphql_query(
            QUERY_GAS_READINGS,
            {"accountNumber": account_number, "pceRef": pce_ref, "first": limit},
        )
