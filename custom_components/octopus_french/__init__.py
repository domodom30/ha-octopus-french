...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # Vérifier si l'entrée existe déjà (évite les doublons)
    ...
    # Get scan interval from entry options (spécifique à cette entrée)
    scan_interval = entry.options.get(CONF_SCAN_INTERVAL, DEFAULT_SCAN_INTERVAL)
    
    # Create coordinator with scan interval
    coordinator = OctopusDataUpdateCoordinator(
        hass,
        client,
        entry.data[CONF_ACCOUNT_NUMBER],
        scan_interval  # Passer l'intervalle directement au coordinator
    )
    
    # Store coordinator in hass data (spécifique à cette entrée)
